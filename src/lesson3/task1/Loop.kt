...
fun hasDifferentDigits(n: Int): Boolean {
    val lastDigit = n % 10
    var number = n
    var lastDigitForCheck: Int
    while (number > 0) {
        lastDigitForCheck = number % 10
        if (lastDigit != lastDigitForCheck) return true
        number /= 10
    }
    return false
}
...
fun sin(x: Double, eps: Double): Double {
    ...
    var ressin = sinx
    while (abs(step) > eps) {
        step = (-1.0).pow(i) * sinx.pow(count) / factorial(count)
        ressin += step
        i++
        count += 2
    }
...
fun cos(x: Double, eps: Double): Double {
    ...
    var rescos = 1.0
    while (abs(step) > eps) {
        step = (-1.0).pow(i) * cosx.pow(count) / factorial(count)
        rescos += step
        i++
        count += 2
    }
